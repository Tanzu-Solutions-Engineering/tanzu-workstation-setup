...
# download files
docker exec -t vmw vmw-cli cp tanzu-cli-bundle-windows-amd64.tar.gz
docker exec -t vmw vmw-cli cp kubectl-windows-v1.22.5+vmware.1.exe.gz 
...
gunzip kubectl-windows-v1.22.5+vmware.1.exe.gz
mv kubectl-windows-v1.22.5+vmware.1.exe ~/bin/kubectl.exe

# install tanzu cli
mkdir ~/tanzu-cli
gunzip tanzu-cli-bundle-windows-amd64.tar.gz
tar -xvf tanzu-cli-bundle-windows-amd64.tar -C ~/tanzu-cli
rm tanzu-cli-bundle-windows-amd64.tar 

cp ~/tanzu-cli/cli/core/v0.11.1/tanzu-core-windows_amd64.exe ~/bin/tanzu.exe
tanzu plugin sync

# Above will likely fail.  So Due to [Running Tanzu commands on Windows fails with a certificate error] (https://docs.vmware.com/en/VMware-Tanzu-Kubernetes-Grid/1.3.1/rn/VMware-Tanzu-Kubernetes-Grid-131-Release-Notes.html#knownissues), add following to ~/.config/tanzu/tkg/config.yaml
...
gunzip ~/tanzu-cli/cli/imgpkg-windows-amd64-v0.18.0+vmware.1.gz
cp ~/tanzu-cli/cli/imgpkg-windows-amd64-v0.18.0+vmware.1 ~/bin/imgpkg.exe
imgpkg version
gunzip ~/tanzu-cli/cli/kapp-windows-amd64-v0.42.0+vmware.1.gz
cp ~/tanzu-cli/cli/kapp-windows-amd64-v0.42.0+vmware.1 ~/bin/kapp.exe
kapp version
...
kbld version
gunzip ~/tanzu-cli/cli/ytt-windows-amd64-v0.35.1+vmware.1.gz
cp ~/tanzu-cli/cli/ytt-windows-amd64-v0.35.1+vmware.1 ~/bin/ytt.exe
ytt version
...
curl -L -o ~/bin/yq.exe https://github.com/mikefarah/yq/releases/download/v4.21.1/yq_windows_amd64.exe
yq -V

# download tmc cli to bin directory
curl -L -o ~/bin/tmc.exe https://tmc-cli.s3-us-west-2.amazonaws.com/tmc/0.4.3-898aa74f/windows/x64/tmc.exe
tmc version
...
curl -LO https://github.com/buildpacks/pack/releases/download/v0.24.0/pack-v0.24.0-windows.zip
unzip pack-v0.24.0-windows.zip
mv pack.exe ~/bin/
rm pack*
pack version

curl -LO https://github.com/vmware/govmomi/releases/download/v0.27.4/govc_Windows_x86_64.zip
...
pivnet download-product-files --product-slug='build-service' --release-version='1.4.2' --product-file-id=1082453 --download-dir ~/bin
mv ~/bin/kp-windows-0.4.2.exe ~/bin/kp.exe
